...
public class RecruitArticleDetailPresenter extends BasePresenterImpl<ArticleDetailContract.View> implements ArticleDetailContract.Presenter {

    public RecruitArticleDetailPresenter(ArticleDetailContract.View view) {
        super(view);
    }
...
//                        element.attr("href", "");
//                    }
                    LogUtils.e("手机", body.html());

                    Element GuideRules = body.select("#GuideRules").first();
//                    Element contentbody = body.select("#contentbody").first();
//                    Element showtag = body.select("#showtag").last();
                    Element Banner = body.select(".Banner").last();

                    if (mView != null) {
                        mView.showNewsContent("</br>" + GuideRules.html() + Banner.html()
//                                + (author.html() + contentbody.html() + showtag.html())
