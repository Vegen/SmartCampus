package com.itculturalfestival.smartcampus;

/**
 * Created by vegen on 2018/3/7.
 */

public class Constant {
    public static final String BmobApplicationId = "329abfb679b7560b6d3bc80ef9204d2c";

    public static final int NEWS_TYPE_FOCUS = 1;            // 要闻
    public static final int NEWS_TYPE_COMPREHENSIVE = 2;    // 综合
    public static final int NEWS_TYPE_FLASH = 0;            // 快讯
    public static final int NEWS_TYPE_OTHER = 3;            // 其他

    public static final int MESSAGE_RECRUIT = 0;            // 招生
    public static final int MESSAGE_EMPLOYMENT = 1;         // 就业

    public static final int NOTICE = 0;                     // 招聘公告
    public static final int FAIR = 1;                       // 招聘会
    public static final int CONFERENCE = 2;                 // 宣讲会
    public static final int JOB = 3;                        // 岗位信息
    public static final int IN_SCHOOL = 4;                  // 校内招聘
    public static final int OUT_SCHOOL = 5;                 // 校外招聘
}
